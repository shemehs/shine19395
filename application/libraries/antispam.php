...
class Antispam {
	...
	*/
	...
	var $img_path 		=   './assets/captcha/image/'; // Image path
	var $img_width 		= 	120;   	// CODE_WIDTH
	var $img_height 	= 	30;		// CODE_HEIGHT	

	var $font_name		= 	FALSE;		// test.ttf
	var $font_path 		= 	'./assets/captcha/fonts';  // PATH_TTF	
	...
	function __construct(){
		...
		$this->deleteExpiredcaptchaphotos();
	...
	function get_antispam_image( $override = array() ) {
		...
		$img_name =  sha1($code).'-'.$this->now . '.jpg';
		// $img_name =  $now . '.jpg';
		...
		/**my variable**/
 	   $image_source = $this->img_url . $img_name;
	   /*        */
		ImageDestroy($img);
		
		// return array('id' => sha1("captcha".$code),'word' => $code, 'time' => $this->now, 'image' => $img_markup, 'image_src' => $image_source, 'image_name' => $img_name);
		return array('id' => sha1("captcha".$code),'word' => $code, 'time' => $this->now, 'image_src' => $image_source, 'image_name' => $img_name);
	...
	function deleteExpiredcaptchaphotos(){
		$logm = "";
		list($usec, $sec) = explode(" ", microtime());
		$this->now = ((float)$usec + (float)$sec);
		/**********************************/
		...
		while ($filename = @readdir($current_dir))
		{
			...
			if ($filename != "." and $filename != ".." and $filename != "index.html")
			{
				...
				if($dashpos===false){
					...
					// $logm .= " dash is missing ";
				
				}else{
					// $logm .= " dash is present ";
					$newfilename = substr($filename,$dashpos+1,strlen($filename)-1);
					// $logm .= " new filename : -- ".$newfilename." --";
					// $dotpos = strpos($filename,'.');
					// $name = str_replace(".jpg", "", $filename);
					$name = str_replace(".jpg", "", $newfilename);
					// $logm .= " name : -- ".$name." -- ";
					if (($name + $this->expiration) < $this->now)
					{
						// @unlink($img_path.$filename);
						@unlink($this->img_path.$filename);
						// $logm .= 'expired captcha picture deleted.'.$this->img_path.$filename;
						// log_message('debug', 'expired captcha picture deleted.'.$this->img_path.$filename);
					}else{
						// $logm .= 'captcha picture not yet expired .'.$this->img_path.$filename;
						// log_message('debug', 'captcha picture not yet expired .'.$this->img_path.$filename);
					}
				}
			}else{
				// $logm .= " not an image ";
		...
		// log_message("error",$logm);
		@closedir($current_dir);
/*************************/
	
	}
	function deleteThisCaptcha($filename=false){

		if($this->checkCaptchaPhotoexist($filename)){
		
			@unlink($this->img_path.$filename);
			log_message('debug', 'expired captcha picture deleted.');
			
		}
		return;
	
	}
	function checkCaptchaPhotoexist($filename=false){
	
		if($filename){
		
			if(file_exists($this->img_path.$filename)){
