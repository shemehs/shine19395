...
<html lang="en">
	...
	<body onLoad="" onResize="" class="<?php echo (get_page_content("top_nav"))?'with-top-nav':'without-top-nav'; ?>">
		...
		<div class="<?php echo (get_page_content("container")?get_page_content("container"):"container-fluid"); ?>">
			<div id="page-body-content-wrapper" class="<?php echo (get_page_content("top_nav"))?((get_page_content("collapse_side_nav")&&is_active("collapse_side_nav_link"))?"with-collapse-side-nav":"without-collapse-side-nav"):"without-collapse-side-nav"; ?>  <?php echo (get_page_content("page_footer"))?"with-page-footer":"without-page-footer"; ?> <?php echo (get_page_content("page_header"))?"with-page-header":"without-page-header"; ?>">
				...
				<?php
					if(get_page_content("top_nav")){
							if(get_page_content("top_nav_container")){
								...
								$this->load->view('template/top_nav/container');
							}
						}
				?>
				<?php
					if(get_page_content("page_header")){
						if(get_page_content("page_header_container")){
							...
							$this->load->view('template/page_header/container');
						}
					}
				?>
				<?php
					if(get_page_content("page_top_menu")){
						if(get_page_content("page_top_menu_container")){
							...
							$this->load->view('template/page_top_menu/container');
						}
					}
				?>
						...
						<?php
							...
							if(get_page_content("page_content")){
								if(get_page_content("page_content_container")){
									...
									$this->load->view('template/page_content/container');
								}
							}
				...
				<?php
					if(get_page_content("page_footer")){
						if(get_page_content("page_footer_container")){
							...
							$this->load->view('template/page_footer/container');
						}
					}
				?>
